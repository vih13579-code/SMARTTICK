...
BEGIN TRY
    ...
    (N'Admin'),(N'Shop Manager'),(N'Order Manager'),(N'Warehouse Manager');

    -- Never assume IDENTITY values are 1, 2, 3, 4. Resolve every FK from its business key.
    INSERT INTO dbo.Employees
        (FullName,Birthday,[Password],PhoneNumber,Email,Gender,CreatedDate,[Status],Avatar,RoleID)
    SELECT N'FWatch Administrator','1995-01-01','0e7517141fb53f21ee439b355b5a1d0a',
           '0901000001','admin@fwatch.vn',N'Other',CAST(GETDATE() AS DATE),1,NULL,RoleID
    FROM dbo.Roles WHERE Name=N'Admin'
    UNION ALL
    SELECT N'FWatch Shop Manager','1996-02-02','f09e2fa7b19117d5b6637dcc6388fffa',
           '0901000002','manager@fwatch.vn',N'Other',CAST(GETDATE() AS DATE),1,NULL,RoleID
    FROM dbo.Roles WHERE Name=N'Shop Manager'
    UNION ALL
    SELECT N'FWatch Order Manager','1997-03-03','f09e2fa7b19117d5b6637dcc6388fffa',
           '0901000003','orders@fwatch.vn',N'Other',CAST(GETDATE() AS DATE),1,NULL,RoleID
    FROM dbo.Roles WHERE Name=N'Order Manager'
    UNION ALL
    SELECT N'FWatch Warehouse Manager','1998-04-04','f09e2fa7b19117d5b6637dcc6388fffa',
           '0901000004','warehouse@fwatch.vn',N'Other',CAST(GETDATE() AS DATE),1,NULL,RoleID
    FROM dbo.Roles WHERE Name=N'Warehouse Manager';

    IF (SELECT COUNT(*) FROM dbo.Employees WHERE Email IN
       ('admin@fwatch.vn','manager@fwatch.vn','orders@fwatch.vn','warehouse@fwatch.vn')) <> 4
        THROW 51001, 'Unable to seed employees because one or more required roles are missing.', 1;
    ...
    WHERE c.Email='customer@fwatch.vn';
    ...
    SELECT @OrderCitizen,ProductID,1,10500000 FROM dbo.Products WHERE Model=N'NJ0150-81X'
    UNION ALL SELECT @OrderGShock,ProductID,1,3290000 FROM dbo.Products WHERE Model=N'GA-2100-1A1'
    UNION ALL SELECT @OrderDW,ProductID,1,4690000 FROM dbo.Products WHERE Model=N'DW00100304'
    UNION ALL SELECT @OrderSeiko,ProductID,1,8900000 FROM dbo.Products WHERE Model=N'SRPD55K1';

    DECLARE @ImportCasio INT, @ImportJapan INT;
    INSERT INTO dbo.ImportStocks(EmployeeID,SupplierID,ImportDate,TotalCost,Completed)
    SELECT e.EmployeeID,s.SupplierID,DATEADD(DAY,-30,GETDATE()),48000000,1
    FROM dbo.Employees e CROSS JOIN dbo.Suppliers s
    WHERE e.Email='warehouse@fwatch.vn' AND s.TaxID='FW-CASIO-001';
    SET @ImportCasio = CONVERT(INT,SCOPE_IDENTITY());

    INSERT INTO dbo.ImportStocks(EmployeeID,SupplierID,ImportDate,TotalCost,Completed)
    SELECT e.EmployeeID,s.SupplierID,DATEADD(DAY,-18,GETDATE()),120000000,1
    FROM dbo.Employees e CROSS JOIN dbo.Suppliers s
    WHERE e.Email='warehouse@fwatch.vn' AND s.TaxID='FW-JAPAN-003';
    SET @ImportJapan = CONVERT(INT,SCOPE_IDENTITY());

    INSERT INTO dbo.ImportStockDetails(ImportID,ProductID,ImportQuantity,ImportPrice)
    SELECT @ImportCasio,ProductID,20,1700000 FROM dbo.Products WHERE Model=N'MTP-1302PD-2A2V'
    UNION ALL SELECT @ImportCasio,ProductID,20,2400000 FROM dbo.Products WHERE Model=N'GA-2100-1A1'
    UNION ALL SELECT @ImportJapan,ProductID,10,8500000 FROM dbo.Products WHERE Model=N'NJ0150-81X'
    UNION ALL SELECT @ImportJapan,ProductID,10,7100000 FROM dbo.Products WHERE Model=N'SRPD55K1';

    DECLARE @RateCitizen INT;
    INSERT INTO dbo.ProductRatings(CustomerID,ProductID,OrderID,CreatedDate,Star,Comment,IsDeleted,IsRead)
    SELECT c.CustomerID,p.ProductID,@OrderCitizen,DATEADD(DAY,-20,GETDATE()),5,
           N'Beautiful dial, stable movement, and careful packaging.',0,0
    FROM dbo.Customers c CROSS JOIN dbo.Products p
    WHERE c.Email='customer@fwatch.vn' AND p.Model=N'NJ0150-81X';
    SET @RateCitizen = CONVERT(INT,SCOPE_IDENTITY());

    INSERT INTO dbo.ProductRatings(CustomerID,ProductID,OrderID,CreatedDate,Star,Comment,IsDeleted,IsRead)
    SELECT c.CustomerID,p.ProductID,@OrderDW,DATEADD(DAY,-8,GETDATE()),4,
           N'Elegant design, suitable for daily wear.',0,0
    FROM dbo.Customers c CROSS JOIN dbo.Products p
    WHERE c.Email='minhanh@fwatch.vn' AND p.Model=N'DW00100304';

    INSERT INTO dbo.RatingReplies(EmployeeID,RateID,Answer,IsRead)
    SELECT EmployeeID,@RateCitizen,N'FWatch thanks you for trusting our product.',0
    FROM dbo.Employees WHERE Email='admin@fwatch.vn';
    IF EXISTS (
        SELECT 1 FROM dbo.Employees e
        LEFT JOIN dbo.Roles r ON r.RoleID=e.RoleID
        WHERE r.RoleID IS NULL
    ) THROW 51002, 'Seed validation failed: employee role reference is invalid.', 1;
